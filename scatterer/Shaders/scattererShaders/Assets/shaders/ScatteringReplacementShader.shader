//Renders the scattering "post-processing" effect through shader replacement in a separate buffer, which can be anti-aliased and will always look clean because of no reliance on depth buffer shenanigans
//To be tested if it works, not sure if global shader params will work with textures and stay correct if I don't set them every frame

Shader "Scatterer/ScatteringReplacementShader" {
    SubShader {
...
//            Blend SrcAlpha OneMinusSrcAlpha
            ...
            uniform float _global_alpha;  //to be removed?
