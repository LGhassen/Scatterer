...
fixed4 frag_pcfSoft(v2f i) : SV_Target
{
	...
	//receiverPlaneDepthBias = float3(0.001,0.001,0.001); ////causes minor peterpanning on kerbals walking, otherwise perfect for ~5000 with invProjection														
	//receiverPlaneDepthBias = float3(0.0,0.0,0.001); //flickers a bit around 50000

	float zdepth2 = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv.xy);
	//receiverPlaneDepthBias = float3(0.0,0.0,0.1*Linear01Depth(zdepth2)); //my best or worst hack yet?
																		 //no kerbal peter-panning, flickering around 10000
																		 //also flickers in some edge cases when another ship is loaded nearby
	receiverPlaneDepthBias = float3(0.0,0.0,0.5*Linear01Depth(zdepth2)); //seems to fix all issues, no peter-panning either
	//receiverPlaneDepthBias = float3(0.0,0.0,0.001); //flickers a bit around 50000
