...
namespace scatterer
{
	public class SunFlare : MonoBehaviour
	{
		...
		public void start()
		{
			...
			if ((HighLogic.LoadedScene == GameScenes.FLIGHT) && !(MapView.MapIsEnabled))
			{
				nearCameraHook = (SunflareCameraHook)Scatterer.Instance.ReturnProperCamera(false, false).gameObject.AddComponent(typeof(SunflareCameraHook));
		...
		public void updateProperties()
		{
			...
			if ((HighLogic.LoadedScene == GameScenes.FLIGHT) && !(MapView.MapIsEnabled))
			{
				...
				hitStatus = Physics.Raycast(Scatterer.Instance.ReturnProperCamera(true, false).transform.position,
											 (source.transform.position - Scatterer.Instance.ReturnProperCamera(true,false).transform.position).normalized,
